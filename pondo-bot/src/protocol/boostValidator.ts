...
const hasValidatorBoostedInEpoch = async (
  address: string,
  epoch: bigint
): Promise<boolean> => {
  const validatorBoostMappingValue = await getMappingValue(
    address,
    PONDO_ORACLE_PROGRAM,
    "validator_boosting"
  );
  ...
  const validatorBoost = JSON.parse(
    formatAleoString(validatorBoostMappingValue)
  );
  const boostEpoch = BigInt(validatorBoost.epoch.slice(0, -3));
  console.log(
    `Validator: ${address} has boosted in epoch: ${boostEpoch} by ${validatorBoost.boost_amount}`
  );
...
export const boostValidator = async (
  validatorAddress: string,
  epoch: bigint
): Promise<void> => {
  const hasValidatorBoosted = await hasValidatorBoostedInEpoch(
    validatorAddress,
    epoch
  );
  ...
  console.log(
    `Boosting validator: ${validatorAddress} in epoch: ${epoch} by ${BOOST_AMOUNT}`
  );
  ...
  const txResult = await submitTransaction(
    ...
    PONDO_ORACLE_PROGRAM_CODE,
    "boost_validator",
    [validatorAddress, `${BOOST_AMOUNT}u64`],
    0.085602,
    undefined,
    resolvedImports
  );
